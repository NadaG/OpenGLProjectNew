...
void Mesh::LoadDefaultMesh(const MeshType& meshType)
{
	switch (meshType)
	{
	case TORUS:
...
void Mesh::ParsePositions(string line, int index)
{
	int now = 0;
	while (line.find(' ') != -1)
	{
...
void Mesh::ParseUvs(string line, int index)
{
	int now = 0;
	while (line.find(' ') != -1)
	{
...
void Mesh::ParseNormals(string line, int index)
{
	int now = 0;
	while (line.find(' ') != -1)
	{
...
bool Mesh::ParseFace(string line, int index)
{
	string uvLine;
	string normalLine;
	bool is3Face = true;
...
void Mesh::ParseMtl(string fileName)
{
...
void Mesh::ParseLightColor(string line, char mode)
{
	float colors[3];
	int now = 0;
	while (line.size() > 0)
