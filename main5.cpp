...
//		{
//			defferedShader.SetUniformVector3f("lights[" + to_string(j) + "].position", lightPositions[j]);
//			defferedShader.SetUniformVector3f("lights[" + to_string(j) + "].color", lightColors[j]);
//			const float constant = 1.0; // note that we don't send this to the shader, we assume it is always 1.0 (in our case)
//			const float linear = 0.7;
//			const float quadratic = 1.8;
//			defferedShader.SetUniform1f("lights[" + to_string(j) + "].linear", linear);
//			defferedShader.SetUniform1f("lights[" + to_string(j) + "].quadratic", quadratic);
//			// then calculate radius of light volume/sphere
//			const float maxBrightness = fmaxf(fmaxf(lightColors[j].r, lightColors[j].g), lightColors[j].b);
//			float radius = (-linear + sqrt(linear * linear - 4 * quadratic * (constant - (256.0f / 5.0f) * maxBrightness))) / (2.0f * quadratic);
//			defferedShader.SetUniform1f("lights[" + to_string(j) + "].radius", radius);
//		}
//
//		renderQuad();
//
