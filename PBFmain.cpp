...
void Update(PBFParticle* particles, int row, int col)
{
	...
	for (int T = 0; T < iterTime; T++)
	{
		...
		for (int i = 0; i < particlesNum; i++)
		{
			...
			if (particles[i].predictPosition.y < -border)
				particles[i].predictPosition.y = -border + borderEpsilon;
			if (particles[i].predictPosition.x < -border)
				particles[i].predictPosition.x = -border + borderEpsilon;
			if (particles[i].predictPosition.x > border)
				particles[i].predictPosition.x = border - borderEpsilon;
		}
	}
